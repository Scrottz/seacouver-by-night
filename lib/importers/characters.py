...
from lib.utils import derive_name_from_slug, extract_slug
        ...
        if not char:
            ...
            if old_data:
                ...
                if isinstance(old_aliases, str):
                    old_aliases = (
                        [a.strip() for a in old_aliases.split(",")]
                        if old_aliases
                        else []
                    )
                name = old_data["name"]
                ...
                char_type = questionary.select(
                    "Type:", choices=["NPC", "PC"], default="NPC"
                ).ask()
                ...
                char_id = db.add_character(
                    name=name, slug=slug, char_type=char_type, clan=clan
                )
...
if __name__ == "__main__":
    ...
    import_characters(
        [Path("res/img/npc"), Path("res/img/pc")], Path("data/full_backup.json")
    )
